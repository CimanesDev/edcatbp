    ...
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur shadow-sm border-b border-gray-100">
          ...
          <div className="flex items-center space-x-4">
            {user ? (
              <div className="flex items-center space-x-4">
                ...
                <button onClick={handleLogout} className="text-gray-700 hover:text-gray-900 transition">Logout</button>
              </div>
            ) : (
      ...
      <div className="md:hidden">
        <div className="px-2 pt-2 pb-3 space-y-1">
          {isAdmin ? (
            <>
              <Link to="/admin/products" className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:text-gray-900" onClick={() => setIsMenuOpen(false)}>
                <Package className="w-5 h-5" /> Products
              </Link>
              <Link to="/admin/orders" className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:text-gray-900" onClick={() => setIsMenuOpen(false)}>
                <ClipboardList className="w-5 h-5" /> Orders
              </Link>
            </>
          ) : (
            <>
              <Link to="/products" className="block px-3 py-2 text-gray-600 hover:text-gray-900" onClick={() => setIsMenuOpen(false)}>Products</Link>
              <Link to="/wishlist" className="block px-3 py-2 text-gray-600 hover:text-gray-900" onClick={() => setIsMenuOpen(false)}>Wishlist</Link>
              <Link to="/contact" className="block px-3 py-2 text-gray-600 hover:text-gray-900" onClick={() => setIsMenuOpen(false)}>Contact</Link>
            </>
          )}
          {user ? (
            <>
              ...
              <Link to="/account" className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:text-gray-900" onClick={() => setIsMenuOpen(false)}>
                <User className="w-5 h-5" />
                {getFirstName(user.displayName) || 'Account'}
              </Link>
              <button onClick={() => { handleLogout(); setIsMenuOpen(false) }} className="block w-full text-left px-3 py-2 text-gray-600 hover:text-gray-900">Logout</button>
            </>
          ) : (
            <Link to="/login" className="block px-3 py-2 text-gray-600 hover:text-gray-900" onClick={() => setIsMenuOpen(false)}>Login</Link>
          )}
        </div>
      </div>
    </nav>
